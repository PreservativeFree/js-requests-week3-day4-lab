...
const changeColor = function(e) {
    document.getElementById("say-hello-button").style.background = "black";
...
// CODE HERE
const changeColorback = function(e) {
    document.getElementById("say-hello-button").style.background = "#EFEFEF"
...
const ohMy = () => {
    axios.get('http://localhost:3000/animals')
        .then(response => {
            const myDocBody = document.querySelector('body')		
            for(i=0; i<response.data.length; i++){
				var elem = document.createElement('p')
				elem.textContent = response.data[i]
				myDocBody.appendChild(elem)
			}
        })
        .catch(err => {
            console.log(err)
...
/*
    ...
    Handle the promise returned from the request with a .then, which will take in a callback -- the callback 
    function should print the response.data.
    
    Outside of the function, select the button with the id "repeat-button" and add a click event listener that calls the 
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/:carbon')
    .then(response =>  {
        let repeatParam = document.getElementById('repeat-text')
        repeatParam.textContent = response.data
    })
    .catch(err => {
        console.log(err)
    })
...
const myRepeatButton = document.querySelector("#repeat-button")

myRepeatButton.addEventListener("click", repeatMyParam); //
// PROBLEM 7
/*
...
const getQuery = () => {
        axios.get('http://localhost:3000/query-test?user=john&name=melissa')
...
const myQueryButton = document.querySelector("#query-button")

myQueryButton.addEventListener("click", getQuery)
////////////////
//INTERMEDIATE//
////////////////
...
/* 
    ...
    On each iteration of the loop, create a new p element. Set its textContent equal the string at the current index (i) and
...
// CODE HERE 
const foodInput = function() {
    let body = {
        newFood: "carrot"
    }
    let myFoodInput = document.querySelector('input')
    myFoodInput.textContent = body.newFood;
...
const myFoodButton = document.querySelector("#food-button")

myFoodButton.addEventListener("click", foodInput)